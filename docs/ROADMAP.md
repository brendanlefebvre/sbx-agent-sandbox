...
One more from the PR #2 review, deferred because it needs verification the
...
- **Ship the in-container client as a real file.** It's currently generated by a
  ...
  Note this would be the image's FIRST build-context dependency — there is no
  `COPY`/`ADD` in the Sandboxfile today, and even `sbx-agent-status.sh` is piped
  in from the host at runtime rather than baked. `.gitattributes` already pins
  `*.sh text eol=lf`, which is what keeps a CRLF shebang from shipping.

The other deferred review item — **moving the remaining fixed-name exec keys into
the raceless tier** — was probed and closed on 2026-07-25; see FINDINGS P9. Short
version: `core.alternateRefsCommand` was promoted (empty is a true disable, and it
runs host-side on fetch/pull, which is more reachable than the review assumed);
`core.gitProxy` turned out to be **unpinnable** — multi-valued, first-match-wins,
so a repo-local value beats a `-c` override even a non-empty one — and is instead
neutralized indirectly by pinning `protocol.git.allow=never`, since `git://` is
the only transport it applies to. `diff.external` was added to the denylist
(unreachable via the three verbs, but a known exec key), and `core.externaldiff`
dropped, not being a git key at all.
